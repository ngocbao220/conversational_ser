...
class CDIMMemoryModule(nn.Module):
...
class CDIMSerModel(nn.Module):
    def __init__(self, config: CDIMConfig) -> None:
        super().__init__()
        self.config = config
...
def build_cdim_ser_model(model_cfg: dict, embedding_dim: int) -> CDIMSerModel:
    config = CDIMConfig(
