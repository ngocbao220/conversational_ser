...
class SERBaselineConfig:
    ssl_model_name: str = "microsoft/wavlm-base"
    num_labels: int = 4
...
class SERBaseline(nn.Module):
    def __init__(self, config: SERBaselineConfig) -> None:
        super().__init__()
        self.config = config
        self.ssl_model = AutoModel.from_pretrained(config.ssl_model_name)
        ssl_config = AutoConfig.from_pretrained(config.ssl_model_name)
        self.hidden_size = int(getattr(ssl_config, "hidden_size"))

        if config.freeze_ssl:
    ...
    def _unfreeze_last_layers(self, num_layers: int) -> None:
        ...
        if layers is None:
            raise ValueError("Cannot locate SSL transformer layers for unfreezing.")
...
class MeanEmbeddingBaseline(nn.Module):
    """Linear SER baseline over fixed mean-pooled SSL embeddings."""
...
def build_ser_baseline(model_cfg: dict) -> SERBaseline:
    config = SERBaselineConfig(
        ssl_model_name=str(model_cfg.get("ssl_model_name", "microsoft/wavlm-base")),
        num_labels=int(model_cfg.get("num_labels", 4)),
        pooling=str(model_cfg.get("pooling", "attentive_statistics")),
        dropout=float(model_cfg.get("dropout", 0.2)),
        freeze_ssl=bool(model_cfg.get("freeze_ssl", True)),
        unfreeze_last_n_layers=int(model_cfg.get("unfreeze_last_n_layers", 0)),
    )
    return SERBaseline(config)
