...
class SERBaselineConfig:
    encoder_model_name: str = "microsoft/wavlm-base"
    num_labels: int = 4
...
class SERBaseline(nn.Module):
    ...
    def _unfreeze_last_layers(self, num_layers: int) -> None:
        ...
        if layers is None:
            raise ValueError("Cannot locate encoder transformer layers for unfreezing.")
...
def build_ser_baseline(model_cfg: dict) -> SERBaseline:
    config = SERBaselineConfig(
        encoder_model_name=str(model_cfg.get("encoder_model_name", model_cfg.get("wavlm_model_name", "microsoft/wavlm-base"))),
        num_labels=int(model_cfg.get("num_labels", 4)),
        pooling=str(model_cfg.get("pooling", "attentive_statistics")),
        dropout=float(model_cfg.get("dropout", 0.2)),
        freeze_encoder=bool(model_cfg.get("freeze_encoder", model_cfg.get("freeze_wavlm", True))),
