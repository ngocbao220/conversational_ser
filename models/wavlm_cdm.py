...
class WavLM_CDMSerModel(nn.Module):
    def __init__(self, config: WavLMCDMConfig) -> None:
        super().__init__()
        self.config = config
        self.memory = CDMMemoryModule(
...
def build_wavlm_cdm_ser_model(model_cfg: dict, embedding_dim: int) -> WavLM_CDMSerModel:
    config = WavLMCDMConfig(
    ...
    return WavLM_CDMSerModel(config)
