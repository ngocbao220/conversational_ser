...
Motivation:
The current CIM injects temporal features directly into the same GRU memory input as audio embeddings. This may make temporal features noisy and can hurt CDM. The new design must decouple:
...
models/wavlm_dual_branch_cim.py
...
This branch is CDM-style and only receives h_i.
...
Use the same causal temporal features as existing CIM, but allow grouping later.
