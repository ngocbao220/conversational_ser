...
Implement Experiment 2: WavLM + MAL, with NO TIM.
...
2. Pool frame-level hidden states into an utterance embedding:
   `h_i`.
3. Pass `h_i` through a MAL-style dialogue memory module:
   `m_i, S_i = MAL(h_i, S_{i-1})`.
4. Apply residual update:
   `h_tilde_i = h_i + tanh(alpha) * m_i`.
5. Classify:
   `y_hat_i = classifier(h_tilde_i)`.

MAL module requirements:

* Name: `MALMemoryModule`.
...
* Implement a simple, stable MAL-lite memory first. Acceptable choices:
...
Training mode:
Because MAL state depends on dialogue order, implement one of the following:
Option A:
...
* Use a custom batch sampler that keeps dialogue order.
  Option C:
* Precompute WavLM embeddings first, then train the MAL model dialogue-by-dialogue.
...
* Train MAL on precomputed embeddings sorted by dialogue_id and utterance_id.
