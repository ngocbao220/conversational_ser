...
Implement Experiment 3: WavLM + TIM.
...
Experiment 3 must reuse the same WavLM encoder, pooling, classifier structure, MAL-style residual gate, memory dimension, training setup, and evaluation pipeline as Experiment 2.
...
* TIM uses real temporal interaction features.
* TIM keeps the same MAL-style residual memory interface as Experiment 2.
* No future information leaks into temporal features.
* The model can be compared directly with:

  * WavLM baseline
  * WavLM + MAL without TIM
