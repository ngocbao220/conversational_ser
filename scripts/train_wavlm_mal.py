...
from models.wavlm_mal import WavLM_MALSerModel, build_wavlm_mal_ser_model
...
def main() -> None:
    ...
    if not bool(config.get("precompute", {}).get("enabled", True)):
        raise ValueError("Experiment 2 currently expects precompute.enabled=true for fixed mean-pooled WavLM embeddings.")
    ...
    model = build_wavlm_mal_ser_model(config["model"], embedding_dim=embedding_dim).to(device)
