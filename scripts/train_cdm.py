...
from models.cdm import CDMSerModel, build_cdm_ser_model
...
def class_weights_from_dialogues(dialogues: Sequence[DialogueEmbedding], num_labels: int, device: torch.device) -> torch.Tensor:
    ...
    for dialogue in dialogues:
        valid_labels = dialogue.labels[dialogue.labels >= 0]
        if valid_labels.numel() > 0:
            counts += torch.bincount(valid_labels, minlength=num_labels).float()
...
def make_temporal_features(num_utterances: int, temporal_feature_dim: int, device: torch.device) -> torch.Tensor:
    return torch.zeros((num_utterances, temporal_feature_dim), device=device, dtype=torch.float32)
            ...
            trace_rows.append(
                {
                    ...
                    "is_target_label": bool(gold_id >= 0),
                }
            )
    return trace_rows
...
def main() -> None:
    ...
    parser.add_argument("--config", default="configs/main/wavlm_cdm.yaml")
    ...
    if bool(config.get("cross_session", {}).get("enabled", False)):
        ...
        summary_path = run_cross_session("scripts.train_cdm", args.config)
    ...
    model = build_cdm_ser_model(config["model"], embedding_dim=embedding_dim).to(device)
    ssl_extractor = None
    ...
    append_log(log_path, f"parameters cdm total={counts['total']:,} trainable={counts['trainable']:,}")
    if ssl_extractor is not None:
    ...
    if args.debug_memory_trace:
        ...
        trace_rows = trace_cdm_dialogue_memory(
            model,
            debug_dialogue,
            device,
            temporal_feature_dim=temporal_feature_dim,
