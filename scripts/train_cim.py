...
def main() -> None:
    parser = argparse.ArgumentParser(description="Train WavLM mean-embedding + CIM SER model.")
    ...
    if bool(config.get("cross_session", {}).get("enabled", False)):
        ...
        summary_path = run_cross_session("scripts.train_cim", args.config)
        print(f"cross_session_summary={summary_path}")
        return
    if not bool(config["model"].get("use_temporal_features", False)):
        raise ValueError("CIM ablations require use_temporal_features=true to preserve the CIM architecture.")
