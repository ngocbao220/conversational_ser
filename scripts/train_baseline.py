...
def run_epoch(
    ...
    for dialogue in iterator:
        ...
        with torch.set_grad_enabled(is_train):
            output = model(embeddings=embeddings, labels=None)
...
def main() -> None:
    ...
    if bool(config.get("cross_session", {}).get("enabled", False)):
        ...
        summary_path = run_cross_session("scripts.train_baseline", args.config)
        print(f"cross_session_summary={summary_path}")
        return
    if str(config["model"].get("pooling", config.get("precompute", {}).get("pooling", "mean"))) != str(config.get("precompute", {}).get("pooling", "mean")):
        raise ValueError("Cached baseline requires model.pooling to match precompute.pooling.")
