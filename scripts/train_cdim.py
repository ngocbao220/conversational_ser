...
from utils.config import format_config_tree
...
def evaluate_temporal_subsets(
    ...
    return {name: metrics_for_temporal_rows(subset_rows) for name, subset_rows in subsets.items()}
...
def save_temporal_subset_metrics(
    ...
    with Path(predictions_path).open("r", encoding="utf-8", newline="") as handle:
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Train mean-embedding + CDIM SER model.")
    ...
    append_log(log_path, format_config_tree(config, title="Run config"))
    ...
    save_temporal_subset_metrics(
        ...
        strong_overlap_threshold=float(config.get("evaluation", {}).get("strong_overlap_threshold", 0.25)),
    )
